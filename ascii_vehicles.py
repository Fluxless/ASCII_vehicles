...
def print_car(vehicle, color="#000000"):
        ...
        terminal_height = round(terminal_size.lines * 0.75)
        ...
        available_space = terminal_width - len(vehicle)
        for i in range(400):  # Number of animation frames
            if available_space < 0:
                vehicle = vehicle[:terminal_width]
            clear_screen()  
            n+=2
            for j in range(len(car_lines)):
                spaces = " " * n
                print(spaces + car_lines[j])
            total_len = len(spaces) + max_line_length 
            time.sleep(0.00001) # refresh speed
            if total_len>terminal_width:
...
def print_car_reverse(vehicle, color="#000000"):
    ...
    for i in range(400):  # Number of animation frames
        ...
        for j in range(len(car_lines)):
            spaces = " " * n
            print(spaces + car_lines[j])
        time.sleep(0.0001) #refresh speed
